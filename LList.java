...
class LList<T> implements ListInterface<T> {
   ...
   //     int counter;
    ...
    public void reverse() {
        ...
        int i=1;
        int j=getLength();
        while(i<j)//loop till middle of list
        {
        	T a=getEntry(i);
        	replace(i,getNodeAt(j).getData());//replace left hand side data (of middle) by right hand side data 
        	replace(j,a);//replace right hand side data (of middle) by left hand side data
        	i++;
        	j--;
        }
    ...
    public void cycle() {
    	...
    	int n=1;
        T a=getEntry(1);//save value of first node in a variable
        while(n<getLength())//looping till length of list
        {
     	   replace(n,getNodeAt(n+1).getData());//replacing adjacent elements of list
     	   n++;
        }
        replace(getLength(),a);//replace last element by first in list
    }
}
